/* ══════════════════════════════════════════════════════════════
   script.js — P Ganesh | VLSI & Embedded Systems Portfolio
   Features:
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ─────────────────────────────────────────────────────────
     3. ANIMATED COUNTERS — smoothly count up hero stat numbers
        (only numeric values; skips strings like "4+")
  ───────────────────────────────────────────────────────── */
      ...
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          entry.target.querySelectorAll('.stat-num').forEach(numEl => {
            ...
            // Skip tokens like "4+" that contain non-numeric characters
            if (/^[\d.]+$/.test(raw)) {
  ...
  if (canvas) {
    ...
    function drawCircuit() {
      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (dist < connectDist) {
            ...
            ctx.strokeStyle = `rgba(255, 185, 40, ${alpha})`;
            ctx.lineWidth   = 0.6;
            ctx.beginPath();
      ...
      // Draw node dots
      nodes.forEach(n => {
        ctx.fillStyle = 'rgba(255, 185, 40, 0.35)';
        ctx.beginPath();
  ...
  document.querySelectorAll('.skill-chip').forEach(chip => {
    chip.addEventListener('click', () => {
      ...
      chip.style.boxShadow  = '0 0 14px rgba(255, 185, 40, 0.4)';
      setTimeout(() => {
